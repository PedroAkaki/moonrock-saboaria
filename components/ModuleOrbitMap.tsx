...
const MODULE_NODE_LABELS: Record<string, Omit<OrbitNode, "status">[]> = {
  ...
  "sabao-de-oleo-usado": [
    ...
    { id: "superfat", label: "Superfat", icon: <FlaskConical className="h-5 w-5" />, description: "Margem contra excesso alcalino e variação." },
...
export default function ModuleOrbitMap({ slug, module }: ModuleOrbitMapProps) {
  ...
  // Map to OrbitNodeDef for LearningOrbit
  const orbitDefs: OrbitNodeDef[] = nodes.map((n) => ({
    id: n.id,
    label: n.label,
    icon: n.icon,
    status: n.status,
    ...
    <>
      ...
      <LearningOrbit
        ...
        onNodeClick={(node) => {
          ...
          setActiveNode(activeNode?.id === node.id ? null : full);
        }}
            ...
            {activeNode.children && activeNode.children.length > 0 && (
              <div className="mt-3 space-y-1.5">
                ...
                {activeNode.children.map((child) => {
                  const childStatus: OrbitStatus = activeNode.status === "completed" ? "completed"
                    : activeNode.status === "in-progress" && child.id.endsWith("1") ? "completed"
                    : activeNode.status === "in-progress" && child.id.endsWith("2") ? "in-progress"
                    : "not-started";
                  return (
                    <div key={child.id} className="flex items-center gap-2 text-xs text-moon-300">
                      <span className={`block h-2 w-2 shrink-0 rounded-full ${
                        childStatus === "completed" ? "bg-green-400 border-green-400"
                        : childStatus === "in-progress" ? "bg-purple-400 border-purple-400"
                        : "border-moon-400/50 bg-transparent border"
                      }`} />
                      {child.label}
                    </div>
                  );
