...
const MODULE_NODE_LABELS: Record<string, Omit<OrbitNode, "status">[]> = {
  "base-glicerinada": [
    { id: "seguranca", label: "Segurança", icon: <ShieldCheck className="h-5 w-5" />, description: "Cuidados com calor, utensílios e essências." },
    { id: "temperatura", label: "Temperatura", icon: <Thermometer className="h-5 w-5" />, description: "Controle de fusão sem superaquecer a base." },
    { id: "higroscopia", label: "Higroscopia", icon: <Droplets className="h-5 w-5" />, description: "Entender o suor da glicerina e armazenamento." },
    { id: "bolhas", label: "Bolhas", icon: <Circle className="h-5 w-5" />, description: "Acabamento com álcool e despejo cuidadoso." },
    { id: "aditivos", label: "Aditivos", icon: <FlaskConical className="h-5 w-5" />, description: "Corantes, fragrâncias e limites de uso." },
    { id: "receita", label: "Receita", icon: <BookOpen className="h-5 w-5" />, description: "Aplicar o módulo em uma receita simples." },
    { id: "quiz", label: "Quiz", icon: <HelpCircle className="h-5 w-5" />, description: "Fixar conceitos antes da prática." },
    { id: "checklist", label: "Checklist", icon: <ClipboardCheck className="h-5 w-5" />, description: "Confirmar critérios antes de avançar." },
  ],
  "sabao-de-oleo-usado": [
    ...
    { id: "superfat", label: "Superfat", icon: <FlaskConical className="h-5 w-5" />, description: "Margem de segurança contra soda livre." },
...
function getNodeStatus(
  ...
  if (nodeId === "quiz") {
    ...
    if (isRecordComplete(moduleProgress.quizAnswers, quizCount)) return "completed";
    if (hasAnyRecordValue(moduleProgress.quizAnswers)) return "in-progress";
    return "not-started";
  }

  if (nodeId === "checklist") {
    if (checklistCount === 0) return "not-started";
    if (isRecordComplete(moduleProgress.checklist, checklistCount)) return "completed";
    if (hasAnyRecordValue(moduleProgress.checklist)) return "in-progress";
    return "not-started";
  }

  if (nodeId === "seguranca") {
    if (beforeCount === 0) return moduleProgress.status === "in-progress" ? "in-progress" : "not-started";
    if (isRecordComplete(moduleProgress.beforePractice, beforeCount)) return "completed";
    if (hasAnyRecordValue(moduleProgress.beforePractice)) return "in-progress";
    return moduleProgress.status === "in-progress" ? "in-progress" : "not-started";
  }

  if (moduleProgress.status === "in-progress") return "in-progress";
  return "not-started";
}

function getStatusClass(status: OrbitStatus) {
  if (status === "completed") {
    return "border-green-400/60 bg-green-500/30 text-green-100 shadow-[0_0_24px_rgba(34,197,94,0.30)]";
  }
  if (status === "in-progress") {
    return "border-purple-400/70 bg-purple-500/30 text-purple-100 shadow-[0_0_24px_rgba(168,85,247,0.30)]";
  }
  return "border-moon-400/50 bg-moon-900/70 text-moon-200";
}

export default function ModuleOrbitMap({ slug, title, module }: ModuleOrbitMapProps) {
  ...
  const nodes = useMemo<OrbitNode[]>(() => {
    ...
    return baseNodes.map((node) => ({
      ...node,
      status: getNodeStatus(node.id, slug, module, progress),
    }));
  ...
  return (
    <section className="mb-8 rounded-2xl border border-moon-600 bg-moon-900/70 overflow-hidden shadow-2xl shadow-black/20">
      {/* Header */}
      <div className="border-b border-moon-700 px-4 py-4 text-center">
        <p className="text-[11px] uppercase tracking-[0.22em] text-amber-300">Visão orbital do módulo</p>
        <h2 className="mt-1 text-xl font-bold text-white">{title}</h2>
        <p className="mt-1 text-xs text-moon-400">Toque nos submódulos para ver o que falta concluir.</p>
      </div>
      ...
      <div className="relative mx-auto h-[390px] max-w-[430px] overflow-hidden">
        ...
        {/* Central planet */}
        <div className="absolute left-1/2 top-1/2 z-10 flex aspect-square w-36 -translate-x-1/2 -translate-y-1/2 flex-col items-center justify-center rounded-full border border-amber-300/60 bg-moon-800 text-center shadow-[0_0_44px_rgba(191,149,48,0.35)]">
          <FlaskConical className="mb-2 h-6 w-6 text-amber-200" />
          <span className="px-3 text-sm font-bold leading-tight text-white">{title}</span>
          <span className="mt-1 text-[10px] text-amber-300">Módulo principal</span>
        </div>
        ...
        {nodes.map((node, index) => {
          ...
          return (
            ...
            >
              <span className="mb-1">{node.status === "completed" ? <Check className="h-5 w-5" /> : node.icon}</span>
              <span className="max-w-[4.5rem] text-[10px] font-medium leading-tight">{node.label}</span>
            </button>
          );
        })}
      </div>
        ...
        <div className="flex items-end justify-between gap-4">
          <div className="flex-1">
            ...
            <p className="mt-2 text-xs text-moon-500">
              Você concluiu <span className="text-green-400">{completedCount}</span> de {nodes.length} submódulos.
            </p>
          </div>
