  ...
  FOCUS_FILE="${OUTPUT%.md}-contexto-${TASK_FOCUS}.md"
  {
    ...
    case "$TASK_FOCUS" in
      diario)
        ...
        echo ""
        echo "## Problema atual conhecido"
        echo "- A calculadora salva 'moonrock:calculator:lastFormula:v1' e redireciona para /diario"
        echo "- O formulário existe dentro de /diario (não há rota /diario/novo)"
        echo "- O snapshot da fórmula incluindo óleos agora é preservado"
        echo "- source.type muda para 'calculator' quando importado da calculadora"
        echo ""
        echo "## Critérios de aceite"
        echo "- /diario lista lotes salvos em moonrock:diario:batches:v1"
        echo "- Botão 'Novo Lote' abre formulário minimalista"
        echo "- Fórmula da calculadora preenche o formulário (óleos, NaOH, água, superfat)"
        echo "- Ao salvar, o lote preserva snapshot completo da fórmula (óleos com SAP)"
        echo "- source.type reflete a origem (calculator / free_formula)"
        echo "- npm run build passa"
        echo "- npm run lint passa"
        echo ""
        ...
        echo ""
        echo "## Arquivos fora de escopo"
        echo "- components/ModuleOrbitMap.tsx"
        echo "- components/VisualRoadmap.tsx"
        echo "- components/RoadmapMap.tsx"
        echo "- data/learning-modules.json"
        echo "- data/curriculo.md"
        echo "- app/aprendizado/*"
        echo "- app/roadmap/*"
        echo "- app/oleos/*"
        echo ""
        echo "## Resultado esperado em uma frase"
        echo "Depois da correção, a fórmula calculada deve virar um lote salvo com óleos, SAP, soda, água, superfat, datas de cura e status."
        echo ""
